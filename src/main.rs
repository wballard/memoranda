...
use tracing::{error, info, warn, span, Level};
...
async fn run_cli() -> Result<()> {
    ...
    let cli = Cli::try_parse()
        .map_err(|e| CliError::invalid_argument("command line", e.to_string()))
    ...
    match &cli.command {
        Some(Commands::Doctor { verbose, auto_fix }) => {
            let _cmd_span = span!(Level::INFO, "doctor_command", verbose = verbose, auto_fix = auto_fix).entered();
            info!(verbose = verbose, auto_fix = auto_fix, "Running doctor command");
        ...
        None => {
            ...
            help.run()
                .context("Help command execution failed")?;
        }
    }
