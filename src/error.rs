...
// Convert MemoStoreError to MemorandaError
...
impl McpError {
    ...
    pub fn server_initialization_failed(reason: impl Into<String>) -> Self {
        Self::ServerInitializationFailed { reason: reason.into() }
    }
}

impl CliError {
    pub fn invalid_command(command: impl Into<String>) -> Self {
        Self::InvalidCommand { command: command.into() }
    }
    
    pub fn missing_argument(argument: impl Into<String>) -> Self {
        Self::MissingArgument { argument: argument.into() }
    }
...
mod tests {
    ...
    fn test_memo_error_creation() {
        ...
        assert_eq!(error.to_string(), "Invalid memo format: missing required field");
    ...
    fn test_mcp_error_creation() {
        ...
        assert_eq!(error.to_string(), "Server initialization failed: memo store not found");
    }
    ...
    fn test_cli_error_creation() {
        ...
        assert_eq!(error.to_string(), "Invalid argument: --verbose - expected boolean");
    }
    
    #[test]
    fn test_memoranda_error_creation() {
        let error = MemorandaError::config("missing configuration file");
        assert_eq!(error.to_string(), "Configuration error: missing configuration file");
    ...
    fn test_error_with_source() {
        ...
        assert_eq!(memoranda_error.to_string(), "Storage error: failed to read file");
