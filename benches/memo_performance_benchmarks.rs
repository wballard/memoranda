...
fn bench_large_collection_handling(c: &mut Criterion) {
    ...
    for &size in &[SMALL_COLLECTION_SIZE, MEDIUM_COLLECTION_SIZE, LARGE_COLLECTION_SIZE] {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.iter_batched(
                    ...
                    |(store, _temp_dir)| {
                        black_box(store.list_memos().unwrap())
                    },
                    criterion::BatchSize::SmallInput,
                );
            },
...
fn bench_memo_store_operations(c: &mut Criterion) {
    ...
    group.bench_function("search_memos_string", |b| {
        b.iter(|| {
            black_box(store.search_memos("benchmark").unwrap())
        });
    });
    ...
    group.bench_function("get_memo_by_id", |b| {
        ...
        b.iter(|| {
            black_box(store.get_memo(id).unwrap())
        });
    });
    ...
    group.bench_function("get_all_context", |b| {
        b.iter(|| {
            black_box(store.get_all_context().unwrap())
        });
    });
